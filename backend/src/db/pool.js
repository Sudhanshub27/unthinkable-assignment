...
if (isPgRequested) {
  const isRemotePg = process.env.DATABASE_URL.includes('render.com') ||
  ...
  const pgPool = new Pool({
    ...
    ssl: isRemotePg ? { rejectUnauthorized: false } : (process.env.DATABASE_SSL === 'true' ? { rejectUnauthorized: false } : false),
  });
