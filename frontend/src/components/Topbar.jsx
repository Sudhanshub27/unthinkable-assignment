...
export default function Topbar() {
  ...
  return (
    <header className="sticky top-0 z-50 h-16 bg-paper-card border-b border-line shadow-soft px-4 md:px-8 flex items-center justify-between">
      {/* Left: Logo & Brand */}
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          {dropdownOpen && (
            <div className="absolute right-0 top-12 w-56 bg-paper-card border border-line rounded-lg shadow-card py-2 z-50">
      ...
      {mobileMenuOpen && (
        <div className="md:hidden absolute top-16 left-0 right-0 bg-paper-card border-b border-line shadow-card px-4 py-3 z-40 flex flex-col gap-1">
          {navItems.map((item) => {
            const Icon = item.icon;
            const active = isActive(item.path);
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium transition-colors ${
                  active
                    ? 'bg-terracotta-50 text-terracotta-500 font-semibold'
